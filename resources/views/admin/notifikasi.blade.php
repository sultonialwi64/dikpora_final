...
<head>
    ...
    <style>
        ...
        .status-pending { background-color: #E0A800; color: white; }
...
<body>
                        ...
                        <div class="card-body">
                            <div class="list-group">
                                ...
                                <div class="list-item" data-toggle="modal" data-target="#bookingModal" 
                                    ...
                                    <h5 class="mb-1">Permintaan Peminjaman Ruangan</h5>
                                    <p class="mb-1">
    ...
    <script>
        $('#bookingModal').on('show.bs.modal', function (event) {
            ...
            var penanggungJawab = button.data('penanggung');

            // Isi elemen modal dengan data
            $('#modalRoomName').text(roomName || 'Tidak tersedia');
            $('#modalAgenda').text(agenda || 'Tidak tersedia');
            $('#modalBookingDate').text(date || 'Tidak tersedia');
            $('#modalBookingTime').text(time || 'Tidak tersedia');
            $('#modalJumlah').text(jumlah || 'Tidak tersedia');
            $('#modalPenanggungJawab').text(penanggungJawab || 'Tidak tersedia');
        });
        ...
        $('#bookingModal').on('show.bs.modal', function (event) {
            ...
            $('#acceptButton').off('click').on('click', function () {
                updateBookingStatus(roomId, 'accepted');
            });
            $('#rejectButton').off('click').on('click', function () {
                updateBookingStatus(roomId, 'rejected');
            });
        });
